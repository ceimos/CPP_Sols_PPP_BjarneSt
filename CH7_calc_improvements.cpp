...
double primary()
{
    ...
    switch (t.type){
        ...
        case '-':
            return -primary(); //Deals with leading '-' OR in other words negative numbers.
        case '+':
            return primary(); //useless but someone's gonna try a leading '+' simply because '-' exists.
    }
}
...
int main()
{
    ...
    while(cin){
        ...
        if(t.type=='q'){break;}
        if(t.type=='=')//changed ';' to '=' for printing result
        {
            cout<<val<<'\n';
        }
        else
